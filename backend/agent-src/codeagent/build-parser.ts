...
export function parseBuildErrors(output: string | null | undefined): BuildError[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m) {
      errors.push({
        file: m[1] as string, line: Number(m[2]), col: Number(m[3]),
        code: m[4] as string, message: m[5] as string
      });
      continue;
    }
    ...
    if (m) {
      ...
      errors.push({
        file: m[2] as string, line: 0, col: 0, code: "UNRESOLVED_IMPORT",

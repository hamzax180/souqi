...
export function isStopOrCorrection(prompt: string): boolean {
 ...
 */
export function attachmentIsInstruction(prompt: string, attachedImages: any): boolean {
...
export function isQuestionOrConversational(prompt: string): boolean {
  ...
  if (STOP_OR_CORRECTION.test(p)) return true;
...
export async function executeRun(runId: string, opts: ExecuteRunOpts = {}): Promise<any> {
  ...
  const attachmentMeansWork =
    attachmentIsInstruction(run.prompt, (run.context && run.context.attachedImages) || []);

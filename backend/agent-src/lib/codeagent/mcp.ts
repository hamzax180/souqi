...
/* Everything an MCP server returns is `any` on purpose, and the header
   above says why in more words: a server's reply is DATA SHOWN TO A
   MODEL, never instructions this process obeys. Giving it a precise
   type would suggest we trust its shape; we validate what we use and
   re-qualify every tool name regardless. */

export interface McpToolDef {
  name: string;
  description?: string;
  /* Both spellings, because both are in the wild and the server chooses
     which one it sends. Neither is trusted beyond being forwarded. */
  inputSchema?: any;
  input_schema?: any;
}
...
/** A JSON-RPC message. `id` absent means a notification, and both
    transports take that to mean "no reply is coming". */
...
export interface McpTransport {
  kind: "stdio" | "http";
  send(msg: JsonRpcMessage, timeoutMs?: number): Promise<any>;
...
export function loadConfig(): McpServerConfig[] {
  ...
  if (raw) {
    try {
      ...
      console.warn("[mcp] MCP_SERVERS is not valid JSON — ignoring:", (e as Error).message);
      return [];
    }
  }
  ...
  try {
    ...
    console.warn("[mcp] mcp.json is not valid JSON — ignoring:", (e as Error).message);
    return [];
  }
}

/** Accepts either {mcpServers:{name:{...}}} (Claude Desktop's shape) or a plain array. */
export function normaliseConfig(parsed: any): McpServerConfig[] {
  const servers = [];
 ...
 */
function stdioTransport(cfg: McpServerConfig): McpTransport {
  const child = spawn(cfg.command as string, cfg.args || [], {
  ...
  return {
    ...
    send(msg, timeoutMs) {
      ...
      return new Promise((resolve) => {
        ...
        pending.set(msg.id!, { resolve: (m: any) => { clearTimeout(timer); resolve(m); } });
        try { child.stdin!.write(JSON.stringify(msg) + "\n"); }
        catch (e) {
          clearTimeout(timer); pending.delete(msg.id);
          resolve({ error: { message: "write failed: " + (e as Error).message } });
        }
      });
    },
 ...
 */
function httpTransport(cfg: McpServerConfig): McpTransport {
  ...
  return {
    ...
    async send(msg, timeoutMs) {
      ...
      try {
        ...
        clearTimeout(timer);
        return { error: { message: (e as Error).name === "AbortError" ? "timed out after " + timeoutMs + "ms" : (e as Error).message } };
      }
    },
    close() { closed = true; }
  };
...
export class McpServer {
  ...
  async _rpc(method: string, params?: any, timeoutMs?: number): Promise<any> {
  ...
  async callTool(toolName: string, args?: any): Promise<{ ok: boolean; text: string }> {
...
export const NAME_RE = /^mcp__([a-zA-Z0-9_-]+)__([a-zA-Z0-9_.-]+)$/;
...
export class McpRegistry {
  servers: McpServer[];
  byQualified: Map<string, { server: McpServer; tool: McpToolDef }>;
  ...
  isMcpTool(name: string): boolean { return NAME_RE.test(String(name || "")) && this.byQualified.has(name); }

  async call(qualifiedName: string, args?: any): Promise<{ ok: boolean; text: string }> {
...
export const EMPTY = new McpRegistry([]);

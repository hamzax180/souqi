...
import { TERMINAL, publicRun } from "./worker-service";

/* eslint-disable @typescript-eslint/no-explicit-any */
const wrap = (fn: any) => (req: any, res: any, next: any) => Promise.resolve(fn(req, res)).catch(next);

export function register(app: any, { store, ownerOf, limiter, prepare, onQueued }: any): void {
  app.post("/api/codeagent/runs", limiter, wrap(async (req: any, res: any) => {
    ...
    const request = {
      ...
      conversation: Array.isArray(body.conversation) ? body.conversation.slice(-12).map((t: any) => ({
    ...
    const accepted = (run: any) => res.status(202).json({ runId: run.id, projectId: run.projectId,
  ...
  app.get("/api/codeagent/runs/:id", wrap(async (req: any, res: any) => {
  ...
  app.get("/api/codeagent/runs/:id/events", wrap(async (req: any, res: any) => {
    const owner = await ownerOf(req, res);
  ...
  app.post("/api/codeagent/runs/:id/check-result", wrap(async (req: any, res: any) => {
    const run = await store.getRun(req.params.id, await ownerOf(req, res));
    if (!run) return res.status(404).json({ error: "Run not found" });
    res.status(409).json({ error: "Verification is performed by the build service" });
  }));

  app.post("/api/codeagent/runs/:id/cancel", wrap(async (req: any, res: any) => {
    const owner = await ownerOf(req, res);
    const run = await store.getRun(req.params.id, owner);
    if (!run) return res.status(404).json({ error: "Run not found" });

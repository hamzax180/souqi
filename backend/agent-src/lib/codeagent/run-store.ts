...
export async function getSteps(runId: string, owner?: RunOwner): Promise<any[]> {
  if (!(await getRun(runId, owner))) return [];
  return dbRequired().collection("agent_steps").find(
    { runId }, { sort: { turn: 1 }, projection: { _id: 0 } }
  ).toArray();
}
 ...
 */
export async function recoverToolResult(
  runId: string, toolCallId: string, owner?: RunOwner
): Promise<any> {
  if (!runId || !toolCallId) return null;
  if (!(await getRun(runId, owner))) return null;

  const step = await dbRequired().collection("agent_steps").findOne(
    { runId, "toolResults.tool_call_id": toolCallId }, { projection: { _id: 0 } }
  );
  if (!step) return null;

  const result = (step.toolResults || []).find((r: any) => r && r.tool_call_id === toolCallId);
  ...
  const call = (step.toolCalls || []).find((c: any) => c && c.id === toolCallId) || null;
  return {

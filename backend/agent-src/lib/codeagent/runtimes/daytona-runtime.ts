...
function client() {
  ...
  const { Daytona } = require("@daytona/sdk") as any;
 ...
 */
async function attach(sandboxId: any) {
...
async function listFiles(ws: any, dir: any) {
  const out: any[] = [];
  async function walk(relDir: any) {
...
async function run(ws: any, argv: any, timeoutMs: any) {
  const command = argv.map((a: any) => (/[\s"']/.test(a) ? JSON.stringify(a) : a)).join(" ");
...
async function getPublicPreviewUrl(ws: any, port: any, expiresInSeconds: any) {
...
function extractBodyText(html: any) {
...
async function domSnapshot(ws: any, url: any, timeoutMs: any) {
...
export { create, attach, writeFile, writeBinaryFile, readFile, listFiles, run, snapshot, destroy, startPreview, domSnapshot, extractBodyText, getPublicPreviewUrl, readDist, PREVIEW_PORT };

...
function resolveInWorkspace(ws: any, relPath: any) {
...
async function walk(dir: any, root: any, out: any) {
 ...
 */
async function attach(id: any) {
  if (!id) return null;
  let root;
  try { root = workspaceRoot(id); } catch (e: any) { return null; }
  try {
...
async function writeFile(ws: any, relPath: any, content: any) {
  const full = resolveInWorkspace(ws, relPath);
  await fsp.mkdir(path.dirname(full), { recursive: true });
  await fsp.writeFile(full, content, "utf8");
}

async function readFile(ws: any, relPath: any, fromLine: any, toLine: any) {
  const full = resolveInWorkspace(ws, relPath);
...
async function listFiles(ws: any, dir: any) {
...
function killTree(child: any) {
  if (process.platform === "win32") {
    ...
    try { process.kill(-child.pid, "SIGKILL"); } catch (e: any) { child.kill("SIGKILL"); }
  }
}
...
function childEnv() {
  ...
  for (const k of keep) if (process.env[k] != null) env[k] = process.env[k] as string;
...
function run(ws: any, argv: any, timeoutMs: any) {
...
async function waitForPort(port: any, deadlineMs: any) {
...
async function startPreview(ws: any, timeoutMs: any) {
  assertDevOnly();
  ...
  let log = "";
  child.stdout!.on("data", (d: any) => { log = (log + d).slice(-4000); });
  child.stderr!.on("data", (d: any) => { log = (log + d).slice(-4000); });
  child.on("error", (e: any) => { log += "\n" + e.message; });
...
async function stopPreview(ws: any) {
  const p = previews.get(ws.id);
  if (!p) return;
  previews.delete(ws.id);
  try { killTree(p.child); } catch (e: any) { /* already gone */ }
...
async function getPublicPreviewUrl(ws: any) {
  const p = previews.get(ws.id);
...
async function writeBinaryFile(ws: any, relPath: any, base64: any) {
  const full = resolveInWorkspace(ws, relPath);
  await fsp.mkdir(path.dirname(full), { recursive: true });
...
async function readDist(ws: any) {
  const distRoot = path.join(ws.root, "dist");
  const out: Record<string, string> = {};
  async function walkDist(dir: any) {
    let entries;
...
async function snapshot(ws: any) {

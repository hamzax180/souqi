...
const SCHEMAS: ToolSchema[] = [
  ...
  },
  {
    ...
    function: {
      name: "run_command",
      description:
        "Run one command in the isolated build sandbox and read its output. " +
...
const ALLOWED_COMMANDS: Record<string, Set<string> | null> = {
...
function assertCommandAllowed(command: string): string[] {
  ...
  if (subs && !subs.has(argv[1] as string)) {
...
const TOOLS: ToolEntry[] = [
  ...
  },
  {
    ...
    run(args) {
      ...
      if (!command) return { ok: false, content: "Error: run_command needs a command." };
      ...
      try { assertCommandAllowed(command); }

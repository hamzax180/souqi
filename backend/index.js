...
/* What leaves the document. Images, fonts, video, audio, archives — the
   things with no reason to be base64 inside a row that also has to hold
   the site's markup. Deliberately an allowlist of extensions rather than
   a size threshold: a 900KB bundle.js belongs inline where the serve path
   can read it without a second lookup, and a 3KB favicon belongs with the
   other assets whatever it weighs. */
const PUBLISH_BINARY_RE = /\.(png|jpe?g|gif|webp|avif|ico|bmp|svg|woff2?|ttf|otf|eot|mp4|webm|mp3|wav|ogg|pdf|zip)$/i;
...
app.post("/api/codeagent/:key/publish", codeAgentLimiter, express.json({ limit: "12mb" }), async (req, res, next) => {
  try {
    ...
    /* TEXT STAYS ON THE DOCUMENT; BYTES GO TO THE BLOB STORE.
       The whole dist used to be base64 on the project row, which is why
       the guard above exists at all — a 16MB BSON ceiling with a 12MB cap
       bolted under it, and "nothing here bounds what the model could
       reference". One site with a few photographs in it reaches that, and
       the failure is a publish that refuses rather than a page that loads
       slowly.

       Binary is also the part that has somewhere better to be: it is
       content-addressed in the same store every uploaded photo already
       uses, so it dedupes across republishes and moves to S3 with
       everything else in one configuration change. index.html and the JS
       and CSS stay inline — they are small, and index.html is rewritten
       on the way out (see servePublishedSite). */
    const filesMap = Object.create(null);
    const blobKeys = [];
    for (const f of distFiles) {
      if (!f || typeof f.path !== "string" || typeof f.base64 !== "string") continue;
      if (!PUBLISH_BINARY_RE.test(f.path)) { filesMap[f.path] = f.base64; continue; }

      const bytes = Buffer.from(f.base64, "base64");
      const ext = (f.path.split(".").pop() || "bin").toLowerCase();
      const stored = await blobs.put(bytes, {
        ext: ext, contentType: mimeForPath(f.path), persist: true
      });
      if (!stored.ok) {
        // Inline is still correct, just heavier. A publish must not fail
        // because one asset could not be moved out of the document.
        console.warn("[publish] asset stayed inline (" + f.path + "): " + stored.error);
        filesMap[f.path] = f.base64;
        continue;
      }
      filesMap[f.path] = { blob: stored.key, bytes: stored.bytes, type: mimeForPath(f.path) };
      blobKeys.push(stored.key);
    }

    /* Assets the previous publish referenced and this one does not. Keys
       are content hashes, so an unchanged file keeps its key and is not
       touched here; only something genuinely replaced is collected. */
    const prevFiles = (project.published && project.published.files) || {};
    const stillUsed = new Set(blobKeys);
    for (const v of Object.values(prevFiles)) {
      if (v && typeof v === "object" && v.blob && !stillUsed.has(v.blob)) {
        try { await blobs.remove(v.blob); } catch (e) { /* an orphan is not a failed publish */ }
      }
    }
...
`, { name: "README.md" });
    ...
    /* Write the published dist/ files. Two shapes, same as the serve
       path: a string is base64 inline, an object is a pointer into the
       blob store. An export that silently shipped a dist/ with no images
       in it would be the worst possible place to discover the split. */
    const pubFiles = project.published.files;
    for (const [filePath, entry] of Object.entries(pubFiles)) {
      if (entry && typeof entry === "object" && entry.blob) {
        const got = await blobs.read(entry.blob);
        if (got.ok) archive.append(got.buf, { name: "dist/" + filePath });
        else console.warn("[export] asset missing from the blob store: " + filePath);
        continue;
      }
      archive.append(Buffer.from(entry, "base64"), { name: "dist/" + filePath });
    }
...
async function servePublishedSite(req, res, subPath, projectOverride) {
  try {
    ...
    /* An asset kept out of the document, fetched from the blob store.
       Two shapes live here on purpose: a string is base64 inline, the way
       every publish before this one wrote it, and an object is a pointer.
       Reading both is what keeps already-published sites serving. */
    const entry = files[key];
    if (entry && typeof entry === "object" && entry.blob) {
      const got = await blobs.read(entry.blob);
      if (!got.ok) return res.status(404).send("Not found");
      if (entry.type) res.setHeader("Content-Type", entry.type);
      // The key is a content hash, so this file can never change under it.

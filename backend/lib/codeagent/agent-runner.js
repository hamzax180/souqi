/* GENERATED FROM backend/agent-src/
   Do not edit this file. Edit agent-src/lib/codeagent/agent-runner.ts and run `npm run build:agent`.
...
function streamWatcher(runId, turn) {
    ...
    const flushThinking = () => {
        ...
        queue(() => runStore.appendEvent(runId, "reasoning_delta", { turn, text: (0, redact_1.redact)(text).text }));
    };
    const watcher = (d) => {
        if (d && d.textDelta) {
            pending += d.textDelta;
        ...
        if (d && d.reasoningDelta && thoughtChars < REASONING_MAX_CHARS) {
            ...
            if (Date.now() - lastThinkFlush >= REASONING_FLUSH_MS)
                flushThinking();
        }
        if (d && d.toolName) {
...
async function executeRun(runId, opts = {}) {
    ...
    let messages = [
        {
            ...
            content: (0, model_loop_1.systemPromptFor)(state.mode === "plan" ? "plan" : run.mode) +
            ...
            if (effects) {
                ...
                if (effects.planPresented) {
                    ...
                    return await settle("succeeded", { phase: "planned" }, {
                        ok: true, stopReason: "plan_presented",
    ...
    if (isGeneric && !isQuestionTurn) {
        ...
        const parts = [];
        if (created)
            parts.push("Created " + created + " file" + (created === 1 ? "" : "s"));
        if (modified)
            parts.push((created ? "updated " : "Updated ") + modified + " file" + (modified === 1 ? "" : "s"));
        if (repairedCount > 0) {

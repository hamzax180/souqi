...
/* =================================================================
   ...
   ================================================================= */
const runStore = __importStar(require("./run-store"));
...
exports.DYNAMIC_TOOLS_SCHEMA = DYNAMIC_TOOLS_SCHEMA;
...
function isQuestionOrConversational(prompt) {
    ...
    if (conversationalPhrases.some((pattern) => pattern.test(p)))
        return true;
...
async function executeRun(runId, opts = {}) {
    ...
    if (outer) {
        if (outer.aborted)
            abort.abort(outer.reason);
        else
            outer.addEventListener("abort", () => abort.abort(outer.reason), { once: true });
    }
    ...
    let messages = [
        {
            role: "system",
            content: (0, model_loop_1.systemPromptFor)(run.mode) +
    ...
    if (opts.history && Array.isArray(opts.history)) {
        messages = messages.concat((0, model_loop_1.buildHistory)(opts.history));
    }
    ...
    const codebaseCtx = (0, model_loop_1.buildCodebaseContext)(currentFiles, {
        prompt: run.prompt,
        budget: (0, model_loop_1.codeBudgetChars)({ effort: effort.id, mode: run.mode })
    });
    ...
    async function finishOnDeadline(turn) {
                ...
                if (effects.commandRequested) {
                    ...
                    if (typeof send !== "function") {
                        ...
                        try {
                            ...
                            out = { ok: false, infra: true, reason: e.message };
                        }
                        ...
                        if (out && out.infra)
                            facts.errors.push("sandbox unavailable: " + (out.reason || ""));
                    }
                }
    ...
    // Final validation
    const finalGate = (0, preflight_1.preflight)(currentFiles);
    const diff = (0, diffstat_1.statsFor)(Object.entries(currentFiles).map(([path, content]) => ({ path, content: String(content) })), turnBaseFiles);

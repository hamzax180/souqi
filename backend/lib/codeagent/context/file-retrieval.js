...
function resolveImport(spec, from, files) {
...
function select(files, opts = {}) {
    ...
    for (const [path, content] of Object.entries(files || {})) {
        if (typeof content !== "string")
            continue;
        let score = 0;
        let reason = "fills the remaining budget";
        ...
        if (touched.has(path)) {
            if (score < 80)
                reason = "edited this run";
            score += 80;
        }
        if (adjacent.has(path)) {
            if (score < 40)
                reason = "imported by a file being changed";
            score += 40;
        }
        if (ENTRY_POINTS.includes(path)) {
            if (score < 30)
                reason = "the entry point";
            score += 30;
        }
        if (read.has(path)) {
            if (score < 10)
                reason = "recently read";
            score += 10;
        }
    ...
    for (const s of scored) {
        if (out.length >= maxFiles)
            break;
        if (used + s.bytes > budgetBytes && out.length)
            break;
        out.push(s);
        used += s.bytes;
    }
 ...
 */
function stale(selections, files) {
    const out = [];
    for (const s of selections || []) {
        const content = files[s.path];
        if (content === undefined) {
            out.push({ path: s.path, was: s.hash, now: null });
            continue;
        }
        const now = hashOf(content);
        if (now !== s.hash)
            out.push({ path: s.path, was: s.hash, now });
    }
    return out;
}
/** A line per selection, for the request. Says why, and at what version. */
function manifest(selections) {

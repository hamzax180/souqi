...
exports.PROMPT_VERSION = exports.DEFAULT_EFFORT = exports.EFFORT = exports.MAX_CLARIFYING_QUESTIONS = exports.SYSTEM_PROMPT = exports.TOOLS_SCHEMA = void 0;
exports.quickAssess = quickAssess;
exports.buildPlan = buildPlan;
exports.proposeChanges = proposeChanges;
exports.proposeWithRepair = proposeWithRepair;
exports.proposeWithClientBuild = proposeWithClientBuild;
exports.repairProposal = repairProposal;
exports.assessPrompt = assessPrompt;
exports.buildHistory = buildHistory;
exports.buildCodebaseContext = buildCodebaseContext;
exports.buildImagesBlock = buildImagesBlock;
exports.fixImageUrls = fixImageUrls;
exports.codeBudgetChars = codeBudgetChars;
exports.fitConversation = fitConversation;
exports.effortFor = effortFor;
exports.systemPromptFor = systemPromptFor;
exports.parseToolCalls = parseToolCalls;
exports.validateWriteFileArgs = validateWriteFileArgs;
exports.cacheKey = cacheKey;
exports.clearCache = clearCache;
exports.cacheStatsSnapshot = cacheStatsSnapshot;
exports.applyEditFileArgs = applyEditFileArgs;
exports.validateReadPath = validateReadPath;
exports.reviewBuild = reviewBuild;
...
exports.TOOLS_SCHEMA = TOOLS_SCHEMA;
...
function fixImageUrls(content, allowed) {
    ...
    return content.replace(REMOTE_IMG, (tag, url) => {
        ...
        return tag.replace(/<img\b/i, '<div aria-hidden="true"')
            .replace(/\bsrc\s*=\s*["'][^"']*["']/i, "")
            .replace(/\bclass(Name)?\s*=\s*["']([^"']*)["']/i, (whole, n, cls) => 'className="' + cls + ' bg-gradient-to-br ' + placeholderFor(url) + '"');
    });
}
function validateWriteFileArgs(args, opts) {
    if (!args || typeof args !== "object")
        throw new Error("tool call arguments were not an object");
    ...
    if (p.startsWith("/") || p.includes(".."))
        throw new Error("write_file: \"" + p + "\" is not a safe relative path");
    ...
    if (PROTECTED_PATHS.has(p))
        throw new Error("write_file: \"" + p + "\" is part of the fixed scaffold and cannot be overwritten");
    ...
    if (inSrc && !/\.(tsx?|css)$/.test(p))
        throw new Error("write_file: \"" + p + "\" must be a .ts, .tsx or .css file");
...
function searchCode(files, query, useRegex) {
    ...
    if (!q)
        throw new Error("search_code: \"query\" must be a non-empty string");
    if (q.length > 200)
        throw new Error("search_code: \"query\" is too long");
    let re;
    if (useRegex) {
        try {
            re = new RegExp(q, "i");
            ...
            throw new Error("search_code: \"" + q + "\" is not a valid regular expression: " + e.message);
        }
    }
    else {
    ...
    for (const path of Object.keys(files || {}).sort()) {
        let allowed;
        try {
            allowed = validateReadPath(path);
        }
        catch (e) {
            continue;
        } // the same set a read may reach, and no wider
...
function validateReadPath(path) {
    ...
    if (p.startsWith("/") || p.includes(".."))
        throw new Error("read_file: \"" + p + "\" is not a safe relative path");
...
function validateEditPath(path) {
    ...
    if (p.startsWith("/") || p.includes(".."))
        throw new Error("edit_file: \"" + p + "\" is not a safe relative path");
    if (!/^src\//.test(p))
        throw new Error("edit_file: only files under src/ are allowed, got \"" + p + "\"");
    if (PROTECTED_PATHS.has(p))
        throw new Error("edit_file: \"" + p + "\" is part of the fixed scaffold and cannot be edited");
    if (!/\.(tsx?|css)$/.test(p))
        throw new Error("edit_file: \"" + p + "\" must be a .ts, .tsx or .css file");
    return p;
}
/**
...
function applyEditFileArgs(args, current, opts) {
    if (!args || typeof args !== "object")
        throw new Error("edit_file: arguments were not an object");
    const p = validateEditPath(args.path);
    if (typeof args.find !== "string" || !args.find)
        throw new Error("edit_file: \"find\" must be a non-empty string");
    if (typeof args.replace !== "string")
        throw new Error("edit_file: \"replace\" must be a string");
...
function parseToolCalls(message, mcp, opts) {
    ...
    if (!calls.length)
        return { ok: false, reason: "model returned no tool calls", content: message && message.content };
    const writes = [];
    const mcpCalls = [];
    const suggestions = [];
    ...
    for (let ci = 0; ci < calls.length; ci++) {
        ...
        if (!name) {
            if (isTruncatedTail && writes.length) {
                droppedTail = "(unnamed)";
                break;
            }
        ...
        if (mcp && mcp.isMcpTool(name)) {
            ...
            try {
                ...
                argError = "malformed JSON arguments: " + e.message;
            }
        ...
        if (name !== "write_file")
            return { ok: false, reason: "unexpected tool call: " + name };
        let args;
        try {
            args = JSON.parse(c.function.arguments);
        }
            ...
            if (isTruncatedTail && writes.length) {
                droppedTail = pathFromPartial(c.function.arguments);
                break;
            }
            return { ok: false, reason: "malformed JSON in tool call arguments: " + e.message, raw: c.function.arguments };
        }
        try {
            writes.push(validateWriteFileArgs(args, opts));
        }
        catch (e) {
            if (isTruncatedTail && writes.length) {
                droppedTail = (args && args.path) || null;
                break;
            }
    ...
    if (!writes.length && mcpCalls.length)
    ...
    if (!writes.length)
        return { ok: false, reason: "model returned no write_file calls" };
...
function buildCodebaseContext(files, opts) {
    ...
    const rank = (p) => {
        ...
        if (ask.includes(base) || (stem.length > 3 && ask.includes(stem)))
            return 0;
        if (/(^|\/)(app|main|index)\.[tj]sx?$/i.test(p))
            return 1;
        return 2;
    };
    ...
    for (const pass of [true, false]) {
        for (const r of rows) {
            if (r.vital !== pass || keep.has(r.p))
                continue;
            if (spent + r.line.length + 1 > room)
                continue;
            keep.add(r.p);
    ...
    if (lostShown)
        kept.push("  … and " + lostShown + " more file(s), each shown in full below");
    ...
    if (lostVital)
        kept.push("  … and " + lostVital + " file(s) that could not be listed for space");
...
exports.DEFAULT_EFFORT = DEFAULT_EFFORT;
...
function fitConversation(messages, opts) {
    ...
    if (budget <= 0 || size(msgs) <= budget)
        return { messages: msgs, dropped: 0 };
...
function codeBudgetChars(opts) {
    ...
    const fixed = client.estimateTokens([{ role: "system", content: systemPromptFor(o.mode) }], base.tools) +
...
async function runToolRounds(messages, opts, base) {
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        ...
        const results = await Promise.all(mcps.map(async (c) => {
            if (c.argError)
                return { id: c.id, text: "Error: " + c.argError };
            if (opts.onToolCall) {
                try {
                    opts.onToolCall({ name: c.name, args: c.args });
                }
        ...
        const readResults = reads.map((r) => {
            if (r.argError)
                return { id: r.id, text: "Error: " + r.argError };
            let p;
            try {
                p = validateReadPath(r.path);
            }
            catch (e) {
                return { id: r.id, text: "Error: " + e.message };
            }
            served.add(r.path);
            const content = opts.files ? opts.files[p] : undefined;
            if (opts.onToolCall) {
                try {
                    opts.onToolCall({ name: "read_file", args: { path: p } });
                }
                catch (e) { }
            }
            if (typeof content !== "string") {
        ...
        const searchResults = searches.map(function (sc) {
            if (sc.argError)
                return { id: sc.id, text: "Error: " + sc.argError };
            if (opts.onToolCall) {
                try {
                    opts.onToolCall({ name: "search_code", args: { query: sc.query } });
                }
                catch (e) { /* observability only */ }
            }
            let found;
            try {
                found = searchCode(opts.files || {}, sc.query, sc.regex);
            }
            catch (e) {
                return { id: sc.id, text: "Error: " + e.message };
            }
...
async function attemptOnce(messages, opts) {
    ...
    if (!retryRes.ok)
        return { ok: false, reason: retryRes.reason || "retry call failed" };
    ...
    for (const c of (parsed.ok && parsed.calls) || []) {
        if (!seen.has(c.path))
            merged.push(c);
    }
    return {
...
async function proposeChanges(userPrompt, opts) {
    ...
    if (cached)
        return Object.assign({}, cached, { cached: true, costUsd: 0 });
    const messages = [
...
async function proposeWithRepair({ userPrompt, tools, maxRounds, onRound, mode, byok, thinking, mcp, onToolCall, history }) {
    ...
    for (let round = 0; round <= cap; round++) {
        ...
        for (const c of attempt.calls)
            await tools.write_file(c.path, c.content);
        ...
        if (onRound)
            onRound({ round, ok: build.ok, calls: allCalls, errors: build.ok ? undefined : build.errors });
        if (build.ok) {
...
async function proposeWithClientBuild({ userPrompt, maxRounds, onFiles, onRound, onProposal, mode, effort, byok, thinking, mcp, onToolCall, history, hasExistingEntry, imageUrls, baseFiles, deadlineAt }) {
    ...
    if (cached) {
        ...
        if (onProposal) {
            try {
                onProposal({ round: 0, calls: cached.calls || [] });
            }
            catch (e) { /* observability only */ }
        }
        const build = await onFiles(cached.calls);
        if (onRound)
            onRound({ round: 0, ok: build.ok, calls: cached.calls, errors: build.ok ? undefined : build.errors });
        if (build.ok) {
    ...
    const noteFailure = (errors) => {
        for (const e of errors || []) {
            ...
            if (seenFailures.some((f) => f.code === entry.code && f.message === entry.message))
                continue;
            if (seenFailures.length < 12)
                seenFailures.push(entry);
        }
    };
    ...
    for (let round = 0; round <= cap + entryRounds; round++) {
        ...
        if (!attempt.ok) {
            ...
            if (fallbackBuild.ok) {
                if (onRound)
                    onRound({ round, ok: true, calls: fallbackCalls });
        ...
        if (onProposal) {
            try {
                onProposal({ round: round, calls: attempt.calls || [] });
            }
        ...
        let build;
        if (missingEntry) {
            if (entryRounds < MAX_ENTRY_ROUNDS)
                entryRounds++;
            build = {
                ok: false,
                errors: [{
            ...
            const gate = (0, preflight_1.preflight)(editBase);
            if (gate.hard.length) {
        ...
        if (build.ok) {
            ...
            if (canReview) {
                ...
                if (!verdict.ok) {
                    ...
                    if (onRound)
                        onRound({ round, ok: false, calls: allCalls, errors: build.errors });
                }
            }
        }
        if (build.ok) {
        ...
        const turnStats = (0, diffstat_1.statsFor)(allCalls, turnBase);
...
async function reviewBuild(userPrompt, calls, opts) {
    ...
    try {
        ...
        if (!r || r.ok !== false || !Array.isArray(r.missing))
...
function headline(prompt, max = 58) {
...
exports.MAX_CLARIFYING_QUESTIONS = MAX_CLARIFYING_QUESTIONS;
...
function quickAssess(userPrompt) {
    ...
    if (GREETINGS.has(norm) || GREETINGS.has(collapsed))
        return { clear: false, reply: hi };
    ...
    if (words.length <= 2) {
        ...
        if (REPEATED_CHAR.test(squished))
            return { clear: false, reply: hi }; // HHH, aaaa, zzz
        if (squished.length < 3)
            return { clear: false, reply: hi }; // "ok", "a"
...
async function assessPrompt(userPrompt, opts) {
    ...
    if (quick)
        return Object.assign({ action: "chat" }, quick);
    ...
    try {
        ...
        if (action === "build") {
            ...
            if (brief && brief.length > String(userPrompt || "").trim().length)
                out.brief = brief;
            cacheSet(key, out, res.costUsd || 0);
            return Object.assign({}, out, { costUsd: res.costUsd });
        }

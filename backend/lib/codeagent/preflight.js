...
exports.ALLOWED_PACKAGES = void 0;
...
exports.ALLOWED_PACKAGES = new Set(["react", "react-dom"]);
...
function preflight(files) {
    ...
    for (const [path, contents] of Object.entries(tree)) {
        ...
        if (scaffoldOwned.has(path) && !(files && Object.prototype.hasOwnProperty.call(files, path)))
            continue;
        if (CODE_RE.test(path)) {
            const specs = [];
            let m;
            FROM_RE.lastIndex = 0;
            while ((m = FROM_RE.exec(contents)) !== null)
                specs.push([m[1], m.index]);
            SIDE_EFFECT_RE.lastIndex = 0;
            while ((m = SIDE_EFFECT_RE.exec(contents)) !== null)
                specs.push([m[1], m.index]);
